...
function myfunction(file, progressbar, progress) {
    ...
    var uploader = WebUploader.create({
        swf: '../webuploader/Uploader.swf',
        server: 'HttpHandler/FileHandler.ashx',
        pick: file,//这个id值需要变化
        auto: true,
        chunked: true,
        chunkSize: 20 * 1024 * 1024,
    ...
    uploader.on('uploadSuccess', function (file) { // 整个文件的所有分片都上传成功后，调用该方法
        var data = { 'taskid': taskid, 'filename': file.source['name'], 'FuncName': 'UploadSuccess', 'parentId': '3b1ba8b2-db32-4fdf-a740-9f3bb974bde1', 'description': '这是文件描述' };
