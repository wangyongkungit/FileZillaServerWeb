    ...
    public partial class EmployeeAdd : WebPageHelper
    {
        ...
        private void DataLoad()
        {
            ...
            try
            {
                ...
                if (emp != null)
                {
                    ...
                    if (emp.AVAILABLE == 1)
                    {
                        rblIsDisable.SelectedValue = "1";
                    }
                    else
                    {
                        rblIsDisable.SelectedValue = "0";
                    }
                    // 是否是外分人员
                    if (emp.ISEXTERNAL)
                    {
        ...
        protected void EditEmployee()
        {
            ...
            if (emp != null)
            {
                ...
                emp.ISBRANCHLEADER = rblIsBranchLeader.SelectedValue == "1";
                emp.ISEXTERNAL = rblIsExternal.SelectedValue == "1";
                emp.AVAILABLE = rblIsDisable.SelectedValue == "1" ? 1 : 0;
                emp.TOREGULARDATE = Convert.ToDateTime(txtToRegularDate.Text.Trim());
                decimal userType = 99;
                // 其他用户类型
                if (!string.IsNullOrEmpty(txtUserType.Text.Trim()))
                ...
                if (editFlag)
                {
                    ExecuteScript("alert('编辑成功！');window.location.href='/Pages/Admin/EmployeeAdd.aspx?UserId=" + emp.ID + "';");
                }
                else
                {
